...
server <- function(input, output, session) {
  ...
  output$download <- downloadHandler(
    ...
    content = function(file) {
      list.save(input_clean(),file,type="yaml")
    }
  )
  ...
  observeEvent(input$upload,
               {
                 ...
                 loadedParams <- list.load(input$upload$datapath)
                 ...
                 updateColourInput(session,inputId="geo_color1",value=loadedParams$geo_color1)
                 updateColourInput(session,inputId="geo_color2",value=loadedParams$geo_color2)
